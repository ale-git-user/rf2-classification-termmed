...
public class CommonUtils {
	...
	public static void getLastVersionFile(String exportedFile,
			String previousFile, File outFile,int[] filekey) throws IOException {
		...
		FileInputStream ofis = new FileInputStream(previousFile	);
		InputStreamReader oisr = new InputStreamReader(ofis,"UTF-8");
		BufferedReader obr = new BufferedReader(oisr);
		...
		bw.append("\r\n");
		String spl[];
		HashSet<String> hixs= new HashSet<String>();
		String key;
	...
	public static void getLastVersionRelsReleaseSpecific(String exportedFile,
			String previousFile, File outFile,int[] filekey) throws IOException {
		File ri=new File("RemovedIsas.txt");
		HashSet<String> isasRemoved = new HashSet<String>();
		String line;
		String[] spl;
		if (ri.exists()){

			FileInputStream fisri = new FileInputStream(ri	);
			InputStreamReader isrri = new InputStreamReader(fisri,"UTF-8");
			BufferedReader brri = new BufferedReader(isrri);
			brri.readLine();
			while ((line=brri.readLine())!=null){
				spl=line.split("\t",-1);
				isasRemoved.add(spl[0] + "*" + spl[1]);
			}
		...
		while ((line=obr.readLine())!=null){
			...
			if (!hixs.contains(key)){
				isaRem=false;
				if ( spl[7].equals("116680003") && spl[2].equals("1")){
					String remkey=spl[4] + "*" + spl[5];
					if (isasRemoved.contains(remkey)){
						isaRem=true;
